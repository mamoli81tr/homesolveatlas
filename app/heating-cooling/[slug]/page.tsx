...
export function generateStaticParams() {
  return getAllArticles()
    .filter((a) => a.frontmatter.category === "heating-cooling")
    .map((a) => ({ slug: a.frontmatter.slug }));
}

async function loadArticle(slug: string) {
  return getArticleByHref(`/heating-cooling/${slug}`);
}
...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    noindex: fm.draft,
    ogCategory: "Heating & Cooling",
  });
}

export default async function HeatingCoolingArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
